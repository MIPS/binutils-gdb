...
#include "nanomips-linux-watch.h"

/* Assuming usable watch registers REGS, return the irw_mask of
   register N.  */

uint32_t
nanomips_linux_watch_get_irw_mask (struct pt_watch_regs *regs, int n)
{
  switch (regs->style)
    {
    case pt_watch_style_nanomips:
      return regs->nanomips.watch_masks[n] & IRW_MASK;
    default:
...
uint32_t
nanomips_linux_watch_get_num_valid (struct pt_watch_regs *regs)
{
  switch (regs->style)
    {
    case pt_watch_style_nanomips:
      return regs->nanomips.num_valid;
    default:
      internal_error (__FILE__, __LINE__,
		      _("Unrecognized watch register style"));
    }
...
CORE_ADDR
nanomips_linux_watch_get_watchlo (struct pt_watch_regs *regs, int n)
{
  switch (regs->style)
    {
    case pt_watch_style_nanomips:
      return regs->nanomips.watchlo[n];
    default:
      internal_error (__FILE__, __LINE__,
		      _("Unrecognized watch register style"));
    }
...
void
nanomips_linux_watch_set_watchlo (struct pt_watch_regs *regs, int n,
			      CORE_ADDR value)
{
  switch (regs->style)
    {
    case pt_watch_style_nanomips:
      ...
      regs->nanomips.watchlo[n] = (uint32_t) value;
      break;
    default:
      internal_error (__FILE__, __LINE__,
		      _("Unrecognized watch register style"));
...
uint32_t
nanomips_linux_watch_get_watchhi (struct pt_watch_regs *regs, int n)
{
  switch (regs->style)
    {
    case pt_watch_style_nanomips:
      return regs->nanomips.watchhi[n];
    default:
      internal_error (__FILE__, __LINE__,
		      _("Unrecognized watch register style"));
    }
...
void
nanomips_linux_watch_set_watchhi (struct pt_watch_regs *regs, int n,
				  uint16_t value)
{
  switch (regs->style)
    {
    case pt_watch_style_nanomips:
      regs->nanomips.watchhi[n] = value;
      break;
    default:
      internal_error (__FILE__, __LINE__,
		      _("Unrecognized watch register style"));
...
int
nanomips_linux_read_watch_registers (long lwpid,
				 ...
				 int *watch_readback_valid, int force)
{
  if (force || *watch_readback_valid == 0)
	...
	{
	case pt_watch_style_nanomips:
	  if (watch_readback->nanomips.num_valid == 0)
	    {
	      *watch_readback_valid = -1;
	      return 0;
	    }
...
uint32_t
nanomips_linux_watch_type_to_irw (enum target_hw_bp_type type)
{
  switch (type)
    {
    case hw_write:
...
int
nanomips_linux_watch_try_one_watch (struct pt_watch_regs *regs,
				    CORE_ADDR addr, int len, uint32_t irw)
{
  ...
  for (i = 0; i < nanomips_linux_watch_get_num_valid (regs); i++)
    {
      t_low = nanomips_linux_watch_get_watchlo (regs, i);
      if (t_low != 0 && irw == ((uint32_t) t_low & irw))
	{
	  t_hi = nanomips_linux_watch_get_watchhi (regs, i) | IRW_MASK;
  ...
  for (i = 0; i < nanomips_linux_watch_get_num_valid (regs); i++)
    {
      t_low = nanomips_linux_watch_get_watchlo (regs, i);
      if (t_low == 0
	  && irw == (nanomips_linux_watch_get_irw_mask (regs, i) & irw))
	{
	  if (mask_bits <= (get_reg_mask (regs, i) | IRW_MASK))
	    {
	      /* It fits, we'll take it.  */
	      nanomips_linux_watch_set_watchlo (regs, i, base_addr | irw);
	      nanomips_linux_watch_set_watchhi (regs, i, mask_bits & ~IRW_MASK);
	      return 1;
	    }
	  else
	    {
  ...
  if (free_watches > 1)
    {
      ...
      regs_copy = *regs;
      for (i = 0; i < nanomips_linux_watch_get_num_valid (&regs_copy); i++)
	{
	  t_low = nanomips_linux_watch_get_watchlo (&regs_copy, i);
	  ...
	  if (t_low == 0 && irw == (t_hi & irw))
		...
		segment_len = break_addr - addr;
	      ...
	      nanomips_linux_watch_set_watchlo (&regs_copy, i,
					    (addr & ~mask_bits) | irw);
	      nanomips_linux_watch_set_watchhi (&regs_copy, i,
					    mask_bits & ~IRW_MASK);
	      if (break_addr >= addr + len)
		{
		  *regs = regs_copy;
...
void
nanomips_linux_watch_populate_regs (struct nanomips_watchpoint *current_watches,
				    struct pt_watch_regs *regs)
{
  ...
  for (i = 0; i < nanomips_linux_watch_get_num_valid (regs); i++)
    {
      nanomips_linux_watch_set_watchlo (regs, i, 0);
      nanomips_linux_watch_set_watchhi (regs, i, 0);
    }

  w = current_watches;
  while (w)
    {
      uint32_t irw = nanomips_linux_watch_type_to_irw (w->type);

      i = nanomips_linux_watch_try_one_watch (regs, w->addr, w->len, irw);
