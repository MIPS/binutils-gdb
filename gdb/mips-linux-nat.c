  ...
  for (regi = 0; regi < 32; regi++)
   {
     ...
     regcache_raw_supply (regcache, mips_regnum (gdbarch)->fp0 + regi, buf);
   }
       else
  for (regi = 0; regi < 32; regi++)
   regcache_raw_supply (regcache, mips_regnum (gdbarch)->fp0 + regi,
        (char *) w_regs[regi]);

       if (iovec.iov_len >= 32*16 + 4)
  regcache_raw_supply (regcache, mips_regnum (gdbarch)->fp_implementation_revision,
      (char *) (w_regs + 32) + 0);
       if (iovec.iov_len >= 32*16 + 8)
  regcache_raw_supply (regcache, mips_regnum (gdbarch)->fp_control_status,
      (char *) (w_regs + 32) + 4);
       if (iovec.iov_len >= 32*16 + 12)
  regcache_raw_supply (regcache, mips_regnum (gdbarch)->msa_ir,
      (char *) (w_regs + 32) + 8);
       if (iovec.iov_len >= 32*16 + 16)
  regcache_raw_supply (regcache, mips_regnum (gdbarch)->msa_csr,
      (char *) (w_regs + 32) + 12);
				      ...
				      struct regcache *regcache, int regno)
{
  ...
  for (regi = 0; regi < 32; regi++)
   {
     regcache_raw_collect (regcache, mips_regnum (gdbarch)->fp0 + regi, buf);
  ...
  for (regi = 0; regi < 32; regi++)
   regcache_raw_collect (regcache, mips_regnum (gdbarch)->fp0 + regi,
  (char *) w_regs[regi]);

       regcache_raw_collect (regcache, mips_regnum (gdbarch)->fp_implementation_revision,
     (char *) (w_regs + 32) + 0);
       regcache_raw_collect (regcache, mips_regnum (gdbarch)->fp_control_status,
     (char *) (w_regs + 32) + 4);
       regcache_raw_collect (regcache, mips_regnum (gdbarch)->msa_ir,
     (char *) (w_regs + 32) + 8);
       regcache_raw_collect (regcache, mips_regnum (gdbarch)->msa_csr,
     (char *) (w_regs + 32) + 12);
