 				...
 				struct address_space *aspace, CORE_ADDR pc)
{
  ...
  /* Assume all atomic sequences start with a ll/lld instruction. For mips_isa6,
     instructions LL and LLD differ in only least significant bit, so we use
     only LL opcode to check for appropriate mips_isa6 atomic sequence. */
  if (((!is_mipsrs6) &&
       (itype_op (insn) != LL_OPCODE && itype_op (insn) != LLD_OPCODE)) ||
      ((is_mipsrs6) && ((insn & LL_ISA6_OPCODE) != LL_ISA6_OPCODE)))
    return 0;
  ...
  for (insn_count = 0; insn_count < atomic_sequence_length; ++insn_count)
      ...
      if (is_branch)
	{
      ...
      if ((!is_mipsrs6 && (itype_op (insn) == SC_OPCODE
                       || itype_op (insn) == SCD_OPCODE)) ||
          (is_mipsrs6 && ((insn & SC_ISA6_OPCODE) == SC_ISA6_OPCODE)))
	break;
    }
