...
{
  0x7860, 0x0000,	/* addiupc $2, (.got.plt entry) - .	*/
  0xff22, 0x0000,	/* lw $25, 0($2)			*/
  0x0f02,		/* move $24, $2				*/
  0x4723		/* jrc $25				*/
};
			       ...
			       bfd_boolean save_addend)
{
  ...
  /* Calls from 16-bit code to 32-bit code and vice versa require the
     ...
     acceptable.  */
  *cross_mode_jump_p = (!MIPSR6_P (abfd)
			&& !info->relocatable
...
mips_finish_exec_plt (bfd *output_bfd, struct bfd_link_info *info)
{
  ...
  if (ABI_64_P (output_bfd))
    plt_entry = htab->compact_branches ? mips_n64_exec_plt0_entry
				       : mips_n64_exec_plt0_entry_compact;
  else if (ABI_N32_P (output_bfd))
    plt_entry = htab->compact_branches ? mips_n32_exec_plt0_entry
				       : mips_n32_exec_plt0_entry_compact;
  else if (!htab->plt_header_is_comp)
    plt_entry = htab->compact_branches ? mips_o32_exec_plt0_entry
				       : mips_o32_exec_plt0_entry_compact;
  else if (htab->insn32)
    plt_entry = MIPSR6_P (output_bfd) ? micromips_insn32_o32_exec_plt0_entry
				      : micromips_insn32_o32_exec_plt0_entry_compact;
  else
    plt_entry = MIPSR6_P (output_bfd) ? micromips_o32_exec_plt0_entry
				      : micromips_o32_exec_plt0_entry_compact;
