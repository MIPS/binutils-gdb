 ...
 public:
  Nanomips_relobj(const std::string& name, Input_file* input_file, off_t offset,
    ...
    : Sized_relobj_file<size, big_endian>(name, input_file, offset, ehdr),
      ...
      local_symbol_is_function_(), processor_specific_flags_(0),
  ...
  class Relocate_comdat_behavior
  {
   public:
    ...
    get(const char* name)
    {
      ...
      return default_behavior.get(name);
    }
  };
    ...
    case elfcpp::R_NANOMIPS_JALR16:
      {
        ...
        if (!this->template check_overflow<11>(value, CHECK_SIGNED))
          ...
          return TT_PCREL16;
        else if (!this->template check_overflow<26>(value, CHECK_SIGNED))
          // Transform into balc.
          return TT_PCREL32;
        else if (xlp)
    ...
    bool no_old_padding)
{
  ...
  relobj->adjust_symbols(address, pnis->shndx(), count);
}
      ...
      if (is_defined_in_discarded_section)
        {
          ...
          if (comdat_behavior == CB_SKIP)
            continue;
          else if (comdat_behavior == CB_PRETEND)
            {
